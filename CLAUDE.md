...
**PayPal owns the price; this app only renders it.** Each entry in `PAYPAL_PLANS` stores the plan id *and* the USD price that PayPal plan was created at, and `checkoutBlockReason()` in `financials.js` refuses to open checkout unless that price equals the displayed `PLANS[key].usd`. Changing a price in `PLANS` without creating a new PayPal plan therefore disables checkout rather than charging the old amount. **Never resolve a block by loosening the check** — create the PayPal plan and record its real price. A PayPal subscription plan's price cannot be edited in place, so a price change always means a new plan id.

**Two offers, not three tiers:** `software` (Command Ledger, $99/mo or $990/yr, self-serve) and `advisory` (Command Advisory, $3,000/mo, 6 seats, adds the monthly call and board report). `DIAGNOSTIC` ($750 one-time) is sold by email, not checkout, because one-time charges need PayPal's Orders API which is not wired. Profiles created under the retired Essentials/Pro/Elite pricing are mapped by `resolvePlanKey()`, so `PLANS[plan]` is never undefined — keep that resolver as long as any legacy row or subscription can exist.
